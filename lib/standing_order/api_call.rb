...
module Figo
  ...
  def get_standing_order(standing_order_id, account_id = nil, cents = false)
    if account_id.nil?
      path =  "/rest/standing_orders/#{standing_order_id}?cents=#{cents}"
    else
      path = "/rest/accounts/#{account_id}/standing_orders/#{standing_order_id}?cents=#{cents}"
    end
  ...
  def remove_standing_orders(account_id = nil)
    if account_id.nil?
      ...
      path = "/rest/accounts/#{account_id}/standing_orders"
    end
